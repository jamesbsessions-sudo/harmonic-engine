...
def _build_voicing(root_semitone: int, chord_type: str) -> list:
    ...
    lh = [36 + root_semitone + i for i in shape['lh']]
    rh = [60 + root_semitone + i for i in shape['rh']]
    return lh + rh
...
def _derive_drum_pattern(groove: dict, syncopation: float) -> dict:
    """
    Build drum hit positions (in beats) from groove profile.
    Returns dict of type → [beat positions]
    """
    kick_sync = groove.get('kick_syncopation', 0.3)
    hat_density = groove.get('hat_density', 0.7)
    half_time = groove.get('half_time_feel', False)

    # Snare: always backbeat
    if half_time:
        snare = [2]
    else:
        snare = [1, 3]

    # Clap: on the 2 (or 2+4 if heavy syncopation)
    clap = [1] if syncopation < 0.5 else [1, 3]

    # Kick: base pattern + syncopated hits based on kick_syncopation score
    if half_time:
        kick = [0]
    elif kick_sync < 0.2:
        kick = [0, 2]                   # straight 4-on-floor feel
    elif kick_sync < 0.4:
        kick = [0, 2.5]                 # kick anticipates beat 3
    elif kick_sync < 0.6:
        kick = [0, 1.5, 3]              # syncopated 3-hit pattern
    else:
        kick = [0, 0.75, 2.5]           # heavy syncopation

    # Hi hats: density controls how many 8th positions get filled
    all_eighths = [i * 0.5 for i in range(8)]  # 0, 0.5, 1, ..., 3.5
    if hat_density >= 0.9:
        hat = all_eighths
    elif hat_density >= 0.6:
        # Every 8th but drop some off-beats (slightly sparse)
        hat = [b for b in all_eighths if b % 1 == 0 or b in [0.5, 1.5, 2.5]]
    elif hat_density >= 0.3:
        # Quarter notes only
        hat = [0, 1, 2, 3]
    else:
        hat = [0, 2]
