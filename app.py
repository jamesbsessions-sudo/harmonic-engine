...
def chord_to_roman_numeral(chord_name: str, key: str) -> str:
    ...
    quality_lower = quality.lower()
    is_minor = ('m' in quality_lower and 'maj' not in quality_lower) or quality_lower.startswith('m')
...
def analyse_drum_patterns(drum_path: str, beat_times, sr=44100):
    ...
    # ── Snare detection (200-1000Hz band + high transient) ─────
    ...
    # Check if snare is on 2 and 4 (backbeat)
    # In a 4-beat bar, beats 2 and 4 would be at positions starting from beat index 1 and 3
    backbeat_snares = 0
    for onset in snare_onsets:
        if len(beat_times) < 2:
            continue
        beat_idx = np.argmin(np.abs(beat_times - onset))
        # Check if this beat is roughly on beat 2 or 4 of a bar
        bar_position = beat_idx % 4
        if bar_position in (1, 3) and abs(beat_times[beat_idx] - onset) < 0.1:
            backbeat_snares += 1

    snare_backbeat_pct = round(backbeat_snares / max(len(snare_onsets), 1) * 100, 1)

    if snare_backbeat_pct > 60:
        snare_style = "Backbeat (2 and 4)"
    elif snare_density < 1:
        snare_style = "Sparse"
    else:
