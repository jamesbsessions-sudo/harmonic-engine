...
def hz_to_note_name(hz):
    ...
    if hz <= 0 or np.isnan(hz):
        return None
    name = librosa.hz_to_note(hz)
    # librosa.hz_to_note returns unicode sharp ♯ — convert to ASCII #
    # so normalise_enharmonic can match it
...
def detect_bass_roots(bass_path: str, beat_frames, sr=44100):
    ...
    # ── Chroma for pitch detection ─────────────────────────────
    # Chroma reliably identifies the correct pitch class in bass range.
    ...
    # ── Onset detection to gate note changes ───────────────────
    # Only update the bass note when there's an actual new attack.
    # On beats without an onset, carry forward the previous note.
    # This prevents overtone ghosts on sustaining beats.
    onset_env = librosa.onset.onset_strength(y=y_bass, sr=sr)
    onset_frames_detected = librosa.onset.onset_detect(
        y=y_bass, sr=sr, onset_envelope=onset_env, backtrack=True,
    )
    onset_times = librosa.frames_to_time(onset_frames_detected, sr=sr)
    ...
    for i in range(beat_chroma.shape[1]):
        beat_time = float(beat_times_local[i]) if i < len(beat_times_local) else 0

        # Check if there's an onset near this beat
        beat_duration = 0.5
        if i + 1 < len(beat_times_local):
            beat_duration = float(beat_times_local[i + 1] - beat_times_local[i])
        has_onset = any(
            abs(beat_time - ot) < beat_duration * 0.6
            for ot in onset_times
        )

        # Get chroma's best guess for this beat
        frame = beat_chroma[:, i]
        if frame.sum() > 0:
            chroma_note = NOTE_NAMES[frame.argmax()]
        else:
            chroma_note = None

        if has_onset and chroma_note:
            # New note attack — trust chroma
            last_good_note = chroma_note
            bass_notes.append(chroma_note)
        elif chroma_note and chroma_note != last_good_note and last_good_note is None:
            # First note in the track
            last_good_note = chroma_note
            bass_notes.append(chroma_note)
        else:
            # No onset — bass is sustaining, carry forward
            bass_notes.append(last_good_note)

    # ── Post-process: remove overtone ghost notes ───────────────
    # Only fix the tightest case: a single-beat note sandwiched between
    # two beats of the SAME note, where the blip is an overtone of that
    # sustained note. This catches Bb/F ghosts within an Ab sustain
    # without touching legitimate bass movement like Db or Gb.
    OVERTONE_INTERVALS = {7, 4, 10}  # p5, M3, m7 — the strongest overtones

    def is_overtone_of(ghost, sustained):
        """Check if ghost note is an overtone-related pitch of sustained."""
        if ghost is None or sustained is None:
            return False
        g = NOTE_TO_NUM.get(ghost, -1)
        s = NOTE_TO_NUM.get(sustained, -1)
        if g < 0 or s < 0:
            return False
        interval = (g - s) % 12
        return interval in OVERTONE_INTERVALS

    cleaned_bass = list(bass_notes)
    for i in range(1, len(cleaned_bass) - 1):
        curr = cleaned_bass[i]
        prev = cleaned_bass[i - 1]
        nxt = cleaned_bass[i + 1]

        # Only replace if sandwiched between the same note (clear sustain)
        # AND the blip is an overtone of that note
        if prev == nxt and curr != prev and is_overtone_of(curr, prev):
            cleaned_bass[i] = prev

    return cleaned_bass, y_bass
...
def get_rmvpe():
    ...
    if rmvpe_extractor is None:
        ...
        rmvpe_extractor = RMVPEPitchExtractor(
            ...
            device="cpu",
            is_half=False,
            voicing_threshold=0.15,  # Raised from 0.03 — cuts noise/false pitch frames
        )
    return rmvpe_extractor


KLANGIO_API_KEY = os.environ.get("KLANGIO_API_KEY", "0xkl-a93cae920b443f0888a48830ac27821a")
KLANGIO_API_URL = "https://api.klang.io"


def _transcribe_with_klangio(audio_path: str, model: str = "vocal") -> str:
    """
    Send an audio file to Klangio's transcription API and return the MIDI bytes.
    Returns path to downloaded MIDI file, or None on failure.
    """
    ...
    try:
        ...
        with open(audio_path, 'rb') as f:
            resp = requests.post(
                f"{KLANGIO_API_URL}/transcription",
        ...
        if resp.status_code != 200:
            print(f"[KLANGIO] Upload failed: {resp.status_code} {resp.text[:200]}")
            return None

        job_info = resp.json()
        job_id = job_info.get("job_id")
        if not job_id:
            print(f"[KLANGIO] No job_id in response: {job_info}")
            return None

        print(f"[KLANGIO] Job created: {job_id}")

        # Step 2: Poll for completion (max ~3 minutes)
        for attempt in range(36):  # 36 × 5s = 180s
            _time.sleep(5)
            status_resp = requests.get(
                f"{KLANGIO_API_URL}/job/{job_id}/status",
                headers={"kl-api-key": KLANGIO_API_KEY},
                timeout=15,
            )
            ...
            print(f"[KLANGIO] Status: {status}")

            if status == "COMPLETED":
                break
            elif status == "FAILED":
                print(f"[KLANGIO] Job failed: {status_resp.json()}")
                return None
        else:
            print("[KLANGIO] Timed out waiting for job")
            return None

        # Step 3: Download MIDI
        midi_resp = requests.get(
            f"{KLANGIO_API_URL}/job/{job_id}/midi",
            headers={"kl-api-key": KLANGIO_API_KEY},
            timeout=30,
        )
        if midi_resp.status_code != 200:
            print(f"[KLANGIO] MIDI download failed: {midi_resp.status_code}")
            return None

        # Save to temp file
        midi_path = audio_path.rsplit('.', 1)[0] + '_klangio.mid'
        with open(midi_path, 'wb') as f:
            f.write(midi_resp.content)

        print(f"[KLANGIO] MIDI saved: {midi_path}")
        return midi_path

    except Exception as e:
        print(f"[KLANGIO] Error: {e}")
        return None


def _parse_midi_to_note_events(midi_path: str):
    """
    Parse a MIDI file into a list of (start_time, end_time, midi_note) tuples.
    Uses mido to read note on/off events.
    """
    ...
    ticks_per_sec = mid.ticks_per_beat * (mido.tempo2bpm(tempo) / 60.0)
    ...
    for track in mid.tracks:
        ...
        for msg in track:
            ...
            if msg.type == 'note_on' and msg.velocity > 0:
                active[msg.note] = time_ticks
            elif msg.type == 'note_off' or (msg.type == 'note_on' and msg.velocity == 0):
                if msg.note in active:
                    start_ticks = active.pop(msg.note)
                    start_s = start_ticks / ticks_per_sec
                    end_s = time_ticks / ticks_per_sec
                    # Skip very low notes (markers, artefacts below C3)
                    if msg.note >= 48:
                        notes.append((start_s, end_s, msg.note))

    notes.sort(key=lambda x: x[0])
    return notes


def _midi_events_to_sequence(note_events):
    """
    Convert MIDI note events to a melody sequence.
    Merges consecutive duplicate notes (same pitch back to back).
    Returns (melody_sequence, timestamped_pitches).
    """
    melody_sequence = []
    timestamped_pitches = []

    for start_time, end_time, midi_note in note_events:
        midi_note = int(midi_note)
        if melody_sequence and midi_note == melody_sequence[-1]:
            continue  # skip consecutive duplicate
        melody_sequence.append(midi_note)
        hz = 440.0 * (2.0 ** ((midi_note - 69) / 12.0))
        timestamped_pitches.append((float(start_time), float(hz)))

    return melody_sequence, timestamped_pitches


def _sequence_to_notes(melody_sequence):
    """Convert MIDI sequence to note name list with ASCII sharps."""
    notes = []
    for midi in melody_sequence:
        try:
...
def _run_basic_pitch(audio_path, onset_threshold=0.5, frame_threshold=0.3,
                     minimum_note_length=80, min_freq=130.0, max_freq=1000.0):
    """Run Basic Pitch with given params, return sorted note events or None. Fallback only."""
    try:
        from basic_pitch.inference import predict as bp_predict
        _model_output, _midi_data, note_events = bp_predict(
            audio_path,
        ...
        if note_events:
            # Convert to (start, end, midi) format
            return sorted(
                [(float(e[0]), float(e[1]), int(e[2])) for e in note_events],
                key=lambda x: x[0]
            )
    except Exception:
        pass
...
def extract_melody(audio: np.ndarray, sr: int = 44100, audio_path: str = None):
    """
    Extract vocal melody using Klangio's Sing2Notes API (vocal-specific CRNN model).
    
    Sends the vocal stem to Klangio's transcription API with model='vocal',
    downloads the resulting MIDI, and parses it into note events.
    
    Falls back to Basic Pitch, then pyin, if Klangio is unavailable.
    
    Returns detailed melody analysis matching the expected output format.
    """
    note_events = None

    # ── Try Klangio API first ──────────────────────────────────
    if audio_path and KLANGIO_API_KEY:
        midi_path = _transcribe_with_klangio(audio_path, model="vocal")
        if midi_path:
            try:
                note_events = _parse_midi_to_note_events(midi_path)
                if note_events:
                    print(f"[MELODY] Klangio returned {len(note_events)} notes")
            except Exception as e:
                print(f"[MELODY] Klangio MIDI parse error: {e}")
            finally:
                # Clean up temp MIDI
                try:
                    os.unlink(midi_path)
    ...
    # ── Fallback: Basic Pitch ──────────────────────────────────
    if not note_events:
        resolve_path = audio_path
        tmp_path = None

        if resolve_path is None and audio is not None:
            try:
                import tempfile, soundfile as sf
                tmp = tempfile.NamedTemporaryFile(suffix='.wav', delete=False)
                sf.write(tmp.name, audio, sr)
                resolve_path = tmp.name
                tmp_path = tmp.name
            except Exception:
                pass

        if resolve_path:
            note_events = _run_basic_pitch(
                resolve_path,
                onset_threshold=0.6,
                frame_threshold=0.3,
            ...
            if note_events:
                print(f"[MELODY] Basic Pitch fallback: {len(note_events)} notes")
    ...
    # ── Fallback: pyin ─────────────────────────────────────────
    if not note_events:
        return _extract_melody_pyin(audio, sr)

    # ── Convert to our format with duplicate merging ───────────
    melody_sequence, timestamped_pitches = _midi_events_to_sequence(note_events)
    melody_notes = _sequence_to_notes(melody_sequence)

    # ── Pitch range ────────────────────────────────────────────
    if melody_sequence:
        min_midi = min(melody_sequence)
        max_midi = max(melody_sequence)
        range_semitones = max_midi - min_midi
        min_note = librosa.midi_to_note(min_midi).replace('♯', '#')
        max_note = librosa.midi_to_note(max_midi).replace('♯', '#')
    else:
        range_semitones = 0
        min_note = None
        max_note = None
    ...
    duration = len(audio) / sr if audio is not None else (note_events[-1][1] if note_events else 1)
    ...
    return {
        ...
        # Internal data for chord-relation analysis and grid quantisation
        "_timestamped_pitches": timestamped_pitches,
        "_melody_sequence": melody_sequence,
    }


def _extract_melody_pyin(audio: np.ndarray, sr: int = 44100):
    """
    Fallback melody extraction using pyin when Basic Pitch isn't available.
    """
    from scipy.signal import medfilt

    # ── Extract f0 from audio via pyin ─────────────────────────
    f0, voiced_flag, voiced_probs = librosa.pyin(
        audio, sr=sr,
        fmin=librosa.note_to_hz('C3'),   # Raised from C2 — C3=131Hz, realistic vocal floor (A#2=117Hz was slipping through G2)
        fmax=librosa.note_to_hz('Bb5'),  # Tightened ceiling (was C6)
        frame_length=4096,               # Increased from 2048 — longer window = more stable pitch on sustained vowels
        hop_length=256,                  # Explicit finer resolution (~6ms at 44100Hz)
    )

    # pyin returns NaN for unvoiced frames — treat as unvoiced
    ...
    # Median filter for pitch stability. With hop=256 at 44100Hz,
    # each frame ≈ 6ms. Kernel=15 ≈ 87ms — compromise between the
    # old 11-frame/hop=512 window and the 21 that over-smoothed.
    if len(raw_midi) > 15:
        smoothed_midi = medfilt(raw_midi, kernel_size=15)
    elif len(raw_midi) > 5:
    ...
    # ── Remove short notes (< 3 frames ≈ 17ms at hop=256) ──────
    # Reduced from 4 to recover short legitimate notes; hop=256 means
    # each frame is ~6ms so 3 frames is still enough to filter jitter
    min_frames = 3
    filtered_runs = []
    ...
    for note in melody_sequence:
        try:
            note_name = librosa.midi_to_note(note).replace('♯', '#')
            melody_notes.append(note_name)
        except Exception:
            pass

    # ── Build timestamped pitches for chord-relation analysis ──
    # pyin hop_length=256 at sr=44100 → ~172fps (finer resolution than default 512)
    pyin_hop = 256
    pyin_fps = sr / pyin_hop
    voiced_indices = np.where(voiced_flag)[0]
    timestamped_pitches = list(zip(
        (voiced_indices / pyin_fps).tolist(),
...
def quantise_melody_to_grid(melody_data, beat_times, tempo, beats_per_bar, key):
    ...
    for note in note_events:
        ...
        # Get note name
        try:
            note_name = librosa.midi_to_note(note["midi"]).replace('♯', '#')
        except Exception:
    ...
    # Build a lead-sheet style readable format grouped by bar
    # e.g. "Bar 1: Eb4(3/16)@1.25 | F4(1/16)@2 | G4(1/16)@2.25 | ..."
    bars = {}
    for n in full_grid:
        bar_num = n.get("bar", 0)
        ...
        beat = n.get("beat", 0)
        sub = n.get("sub", 0)
        beat_pos = f"{beat}" if sub == 0 else f"{beat}.{int(sub * 100):02d}"
        time_s = n.get("time_seconds", 0)
        bars[bar_num].append(
            f"{n['note']}({n['duration']})@{beat_pos} [{time_s:.2f}s]"
        )

    readable_bars = []
    for bar_num in sorted(bars.keys()):
        notes_str = " | ".join(bars[bar_num])
        readable_bars.append(f"Bar {bar_num}: {notes_str}")
    readable_lead_sheet = "\n".join(readable_bars)

    return {
        "grid": full_grid,
        "readable": readable,
        "readable_lead_sheet": readable_lead_sheet,
...
def detect_chord_inversions(chord_timestamps, bass_notes_detected, beat_times):
    """
    Cross-reference chord roots with detected bass notes to identify inversions.
    If the bass note doesn't match the chord root, it's an inversion (slash chord).
    Returns inversion analysis with per-chord detail and summary stats.
    """
    def parse_chord_root(chord_name):
    ...
    for i, bass_note in enumerate(bass_notes_detected):
        if bass_note == "—" or not bass_note or i >= len(beat_times):
            continue

        # Find which chord is active at this beat
        beat_time = beat_times[i] if i < len(beat_times) else 0
        active_chord = None
        for j, ct in enumerate(chord_timestamps):
            next_time = chord_timestamps[j + 1]["time"] if j + 1 < len(chord_timestamps) else float('inf')
            if ct["time"] <= beat_time < next_time:
                active_chord = ct["chord"]
                break
        ...
        chord_root, quality = parse_chord_root(active_chord)
        if not chord_root:
            continue

        total_chords += 1

        # Compare bass note to chord root
        bass_num = NOTE_TO_NUM.get(bass_note, -1)
        root_num = NOTE_TO_NUM.get(chord_root, -2)

        if bass_num != root_num and bass_num >= 0 and root_num >= 0:
            # It's an inversion — determine which
            interval = (bass_num - root_num) % 12
            if interval == 4 or interval == 3:
                inversion_type = "1st"  # Bass is the 3rd
            elif interval == 7:
                inversion_type = "2nd"  # Bass is the 5th
            else:
                inversion_type = "slash"  # Non-standard bass note
            ...
            inversions.append({
                ...
                "slash_notation": f"{active_chord}/{bass_note}",
            })
            inversion_count += 1

    inversion_pct = round(inversion_count / total_chords * 100, 1) if total_chords > 0 else 0

    # Categorise usage
    if inversion_pct > 30:
        usage_label = "Heavy use of inversions"
    elif inversion_pct > 15:
        usage_label = "Moderate use of inversions"
    elif inversion_pct > 5:
        usage_label = "Occasional inversions"
    else:
        usage_label = "Root position dominant"

    return {
        "inversion_pct": inversion_pct,
        "inversion_count": inversion_count,
        "total_chords_checked": total_chords,
        "usage_label": usage_label,
        "inversions": inversions[:24],  # Cap output
    }


# ── Rhythmic feel analysis ───────────────────────────────────────
def analyse_rhythmic_feel(y, sr, beat_times):
    """
    Analyse microtiming characteristics of the track.
    Returns swing ratio, microtiming variance, and push/pull tendency.
    """
    if len(beat_times) < 4:
        return {"error": "Not enough beats for rhythmic feel analysis"}

    # Get onsets from the full mix
    onset_env = librosa.onset.onset_strength(y=y, sr=sr)
    ...
    if len(onset_times) < 8:
        return {"error": "Not enough onsets for rhythmic feel analysis"}

    beat_times = np.array(beat_times)

    # ── Swing ratio ──────────────────────────────────────────────
    # For each pair of consecutive beats, find onsets near the midpoint
    # and measure whether they're early (straight) or late (swung)
    swing_ratios = []
    ...
    for i in range(len(beat_times) - 1):
        ...
        midpoint = beat_start + beat_duration / 2

        # Find onsets in the second half of the first 8th and first half of the second 8th
        # i.e. around the midpoint
        window = beat_duration * 0.25  # Look within +/- 25% of a beat from midpoint
        nearby = onset_times[(onset_times > midpoint - window) & (onset_times < midpoint + window)]

        if len(nearby) > 0:
            # Take the onset closest to the midpoint
            closest = nearby[np.argmin(np.abs(nearby - midpoint))]
            # Ratio: how far through the beat is this onset?
            position_in_beat = (closest - beat_start) / beat_duration
            swing_ratios.append(position_in_beat)

    if swing_ratios:
        avg_swing = float(np.mean(swing_ratios))
        # Convert to swing ratio (e.g. 0.5 = straight, 0.67 = triplet swing)
        swing_ratio = round(avg_swing, 3)

        if avg_swing > 0.58:
            swing_label = "Swung"
        elif avg_swing > 0.53:
            swing_label = "Light shuffle"
        elif avg_swing < 0.47:
            swing_label = "Pushed"
        else:
            swing_label = "Straight"
    ...
    # For each onset, find how far it is from the nearest beat subdivision (16th)
    beat_duration_avg = float(np.median(np.diff(beat_times)))
    sixteenth = beat_duration_avg / 4

    # Build a 16th note grid from beat times
    grid = []
    for bt in beat_times:
        for sub in range(4):
            grid.append(bt + sub * sixteenth)
    grid = np.array(grid)
    ...
    for onset in onset_times:
        if onset < beat_times[0] or onset > beat_times[-1]:
            continue
        nearest_grid = grid[np.argmin(np.abs(grid - onset))]
        deviation_ms = (onset - nearest_grid) * 1000  # in milliseconds
        deviations.append(deviation_ms)

    if deviations:
        timing_variance_ms = round(float(np.std(deviations)), 1)
        mean_deviation_ms = round(float(np.mean(deviations)), 1)

        if timing_variance_ms < 5:
            tightness_label = "Quantised / programmed"
        elif timing_variance_ms < 12:
            tightness_label = "Tight"
        elif timing_variance_ms < 25:
            tightness_label = "Loose / human"
        else:
            tightness_label = "Very loose"

        # Push/pull: negative mean = onsets tend to land before the grid (pushing)
        if mean_deviation_ms < -5:
            push_pull = "Pushing (ahead of beat)"
        elif mean_deviation_ms > 5:
            push_pull = "Laid back (behind beat)"
        else:
            push_pull = "On the grid"
    else:
        timing_variance_ms = 0
        mean_deviation_ms = 0
        tightness_label = "Unknown"
        push_pull = "Unknown"
...
# ── Reverb / space estimation ───────────────────────────────────
def estimate_reverb_space(y, sr):
    """
    Estimate the amount of reverb/space in a mix by looking at:
    1. Spectral decay rate after transients
    2. Ratio of sustained energy to transient energy
    3. Early reflection density approximation
    """
    # Split into transient and steady-state components
    y_harmonic, y_percussive = librosa.effects.hpss(y)

    # Energy ratio: percussive (transient) vs harmonic (sustained/reverb tails)
    transient_energy = float(np.sum(y_percussive ** 2))
    sustained_energy = float(np.sum(y_harmonic ** 2))
    total_energy = transient_energy + sustained_energy

    if total_energy <= 0:
        return {"error": "No audio energy detected"}

    sustained_ratio = sustained_energy / total_energy

    # Spectral flatness — reverb tends to make the spectrum more diffuse/flat
    flatness = librosa.feature.spectral_flatness(y=y)
    avg_flatness = float(np.mean(flatness))

    # Onset strength contrast — dry signals have sharp onsets,
    # reverby signals have softer onset-to-sustain ratios
    onset_env = librosa.onset.onset_strength(y=y, sr=sr)
    if len(onset_env) > 0 and np.max(onset_env) > 0:
        onset_peaks = onset_env[onset_env > np.percentile(onset_env, 75)]
        onset_valleys = onset_env[onset_env < np.percentile(onset_env, 25)]
        if len(onset_valleys) > 0 and np.mean(onset_valleys) > 0:
            transient_contrast = float(np.mean(onset_peaks) / np.mean(onset_valleys))
        else:
            transient_contrast = 10.0
    else:
        transient_contrast = 10.0

    # Combine indicators into a reverb score (0 = bone dry, 1 = drenched)
    # High sustained ratio = more reverb
    # High flatness = more reverb
    # Low transient contrast = more reverb
    reverb_score = 0.0
    reverb_score += min(sustained_ratio, 1.0) * 0.4
    reverb_score += min(avg_flatness * 10, 1.0) * 0.3
    reverb_score += max(0, 1.0 - transient_contrast / 15.0) * 0.3
    reverb_score = round(min(max(reverb_score, 0), 1), 3)

    if reverb_score > 0.7:
        space_label = "Very wet / spacious"
    elif reverb_score > 0.5:
        space_label = "Moderate space"
    elif reverb_score > 0.3:
        space_label = "Tight with some room"
    else:
        space_label = "Dry / close"
    ...
    return {
        ...
        "transient_contrast": round(transient_contrast, 2),
    }
...
def generate_feature_vector(analysis_data: dict) -> list:
    ...
    # 26. Chord inversion usage (0 = all root position, 1 = heavy inversions)
    inversions = analysis_data.get("chord_inversions", {})
    vector.append(min(inversions.get("inversion_pct", 0) / 50, 1))

    # 27. Swing ratio (0 = pushed, 0.5 = straight, 1 = heavy swing)
    feel = analysis_data.get("rhythmic_feel", {})
    vector.append(min(max(feel.get("swing_ratio", 0.5), 0), 1))

    # 28. Timing tightness (0 = quantised, 1 = very loose)
    vector.append(min(feel.get("timing_variance_ms", 10) / 40, 1))

    # 29. Push/pull (0 = pushing, 0.5 = on grid, 1 = laid back)
    deviation = feel.get("mean_deviation_ms", 0)
    vector.append(min(max((deviation + 20) / 40, 0), 1))

    # 30. Reverb/space (0 = dry, 1 = drenched)
    reverb = analysis_data.get("reverb_space", {})
    vector.append(reverb.get("reverb_score", 0.3))
...
def analyse_audio(wav_path: str, song_id: str, keep_stems: bool = False) -> dict:
    ...
    try:
        ...
        if "vocals" in stems:
            try:
                ...
                melody_data = extract_melody(y_vocal, sr, audio_path=stems["vocals"])
            except Exception as e:
        ...
        # ── Normalise melody note names for the key ─────────────
        # librosa.midi_to_note returns sharps by default (D#4, G#3).
        # For flat keys like Eb Major, convert to flats (Eb4, Ab3).
        if "error" not in melody_data and melody_data.get("melody_notes"):
            melody_data["melody_notes"] = [
                normalise_enharmonic(n, best_key) for n in melody_data["melody_notes"]
            ]
            # Also normalise range endpoints
            if melody_data.get("melody_range_low"):
                melody_data["melody_range_low"] = normalise_enharmonic(
                    melody_data["melody_range_low"], best_key)
            if melody_data.get("melody_range_high"):
                melody_data["melody_range_high"] = normalise_enharmonic(
                    melody_data["melody_range_high"], best_key)
    ...
    reverb_space = {}
    try:
        reverb_space = estimate_reverb_space(y, sr)
    except Exception as e:
        reverb_space = {"error": str(e)[:200]}

    # ── Rhythmic feel analysis ─────────────────────────────────
    rhythmic_feel = {}
    try:
        rhythmic_feel = analyse_rhythmic_feel(y, sr, list(beat_times))
    except Exception as e:
        rhythmic_feel = {"error": str(e)[:200]}
    ...
    # ── Chord inversion detection ──────────────────────────────
    chord_inversions = {}
    if chord_timestamps and bass_notes:
        try:
            chord_inversions = detect_chord_inversions(
                chord_timestamps, bass_notes_normalised, list(beat_times)
            )
        except Exception as e:
            chord_inversions = {"error": str(e)[:200]}
    ...
    if melody_grid and "error" not in melody_grid:
        ...
        clean_melody["beat_grid_lead_sheet"] = melody_grid.get("readable_lead_sheet", "")
...
def build_midi_from_analysis(wav_path: str, song_id: str, analysis: dict) -> str:
    ...
    Tracks:
    1. Drums — onset detection + spectral classification from Demucs drum stem
    ...
    # ── Track 1: Drums (onset detection + spectral classification) ──
    ...
    try:
        ...
        if drum_stem_path and os.path.exists(drum_stem_path):
            y_drums, sr_d = librosa.load(drum_stem_path, sr=44100, mono=True)

            # Single onset detection pass on the full drum stem
            onset_env = librosa.onset.onset_strength(y=y_drums, sr=sr_d)
            onset_frames = librosa.onset.onset_detect(
                y=y_drums, sr=sr_d, onset_envelope=onset_env,
                backtrack=True,
            )
            ...
            if max_strength <= 0:
                raise ValueError("All onset strengths zero")

            # Classify each onset by spectral content at that moment
            # Use a short FFT window centred on each onset to get frequency profile
            hop_length = 512
            n_fft = 2048

            # Pre-compute mel spectrogram for efficient lookup
            S = np.abs(librosa.stft(y_drums, n_fft=n_fft, hop_length=hop_length))
            freqs = librosa.fft_frequencies(sr=sr_d, n_fft=n_fft)

            # Define frequency bands
            low_mask = freqs < 200       # Kick region
            mid_mask = (freqs >= 200) & (freqs < 4000)  # Snare region
            high_mask = freqs >= 4000    # Hi-hat region

            print(f"[DRUMS] Detected {len(onset_times)} onsets, classifying by spectral content...")

            for i, (onset_time, strength) in enumerate(zip(onset_times, onset_strengths)):
                # Map onset time to nearest STFT frame
                stft_frame = int(round(onset_time * sr_d / hop_length))
                if stft_frame >= S.shape[1]:
                    stft_frame = S.shape[1] - 1

                # Get spectrum at this onset (average over a few frames for stability)
                frame_start = max(0, stft_frame)
                frame_end = min(S.shape[1], stft_frame + 3)
                spectrum = np.mean(S[:, frame_start:frame_end], axis=1)

                # Calculate energy in each band
                low_energy = float(np.sum(spectrum[low_mask] ** 2))
                mid_energy = float(np.sum(spectrum[mid_mask] ** 2))
                ...
                if low_ratio > 0.6:
                    ...
                    midi_note = HAT_CLOSED
                elif mid_ratio > 0.3 and low_ratio < 0.4:
                    midi_note = SNARE_NOTE
                elif high_ratio > mid_ratio and high_ratio > low_ratio:
                    midi_note = HAT_CLOSED
                ...
                # Map strength to MIDI velocity (30-127)
                normalised = strength / max_strength
                velocity = int(30 + normalised * 97)
                velocity = min(127, max(30, velocity))

                tick = seconds_to_ticks(onset_time)
                drum_events.append((tick, midi_note, velocity))

            print(f"[DRUMS] Classified {len(drum_events)} drum hits (kick/snare/hat)")
        else:
...
async def debug_drums():
    """Check drum transcription dependencies."""
    info = {}
    ...
    info["drum_method"] = "onset_detection_spectral_classification"
...
# ─────────────────────────────────────────────────────────────────────────────
# Drum sample tagger + profile matcher
# ─────────────────────────────────────────────────────────────────────────────

DRUMS_DIR = Path(__file__).parent / "Drums"
LIBRARY_DIR = Path(__file__).parent / "library" / "voicings"
VOICINGS_INDEX_PATH = Path(__file__).parent / "library" / "voicings_index.json"
_voicings_cache = None

def _load_voicings_index():
    global _voicings_cache
    if _voicings_cache is not None:
        return _voicings_cache
    if VOICINGS_INDEX_PATH.exists():
        _voicings_cache = json_module.loads(VOICINGS_INDEX_PATH.read_text())
    return _voicings_cache
...
def _note_name_to_midi(name: str) -> int:
    """Convert e.g. 'C3' or 'Bb4' to MIDI number."""
    # Split letter(s) from octave number
    for i, ch in enumerate(name):
        if ch.isdigit() or (ch == '-' and i > 0):
            note_part = name[:i]
            octave = int(name[i:])
            semitone = NOTE_NAME_TO_SEMITONE.get(note_part, 0)
            return (octave + 1) * 12 + semitone
    return 60

def _transpose_voicing(voicing: dict, target_root_semitone: int) -> dict:
    """
    Transpose a voicing from C (semitone 0) to target_root_semitone.
    Returns new voicing with transposed MIDI notes.
    """
    shift = target_root_semitone  # C is always 0 in the index

    lh_midi = [_note_name_to_midi(n) + shift for n in voicing['lh_notes']]
    rh_midi = [_note_name_to_midi(n) + shift for n in voicing['rh_notes']]

    return {
        'id': voicing['id'],
        'chord_type': voicing['chord_type'],
        'voicing_style': voicing['voicing_style'],
        'spread_rh': voicing['spread_rh'],
        'lh_midi': lh_midi,
        'rh_midi': rh_midi,
        'all_midi': sorted(lh_midi + rh_midi),
    }


def _pick_voicing(chord_type: str, root_semitone: int, prev_rh_midi: list = None) -> dict:
    """
    Pick the best voicing for a chord from the library.
    If prev_rh_midi given, minimise total movement (voice leading).
    Falls back to algorithmic voicing if library not available.
    """
    index = _load_voicings_index()
    if not index:
        # Fallback to algorithmic
        notes = _build_voicing(root_semitone, chord_type)
        return {
            'id': f'{chord_type}_generated',
            'chord_type': chord_type,
            'voicing_style': 'close',
            'spread_rh': 0,
            'lh_midi': [n for n in notes if n < 48],
            'rh_midi': [n for n in notes if n >= 48],
            'all_midi': notes,
        }

    candidates = [v for v in index['voicings'] if v['chord_type'] == chord_type]
    if not candidates:
        notes = _build_voicing(root_semitone, chord_type)
        return {
            'id': f'{chord_type}_generated',
    ...
    # Transpose all candidates to target key
    transposed = [_transpose_voicing(v, root_semitone) for v in candidates]

    if prev_rh_midi:
        # Score by total voice leading distance (sum of nearest-note movements)
        def vl_score(v):
            total = 0
            for rh_note in v['rh_midi']:
                nearest = min(abs(rh_note - p) for p in prev_rh_midi)
                total += nearest
            return total
        transposed.sort(key=vl_score)
        # Pick from top 3 to add some variety
        import random as _rand
        pool = transposed[:min(3, len(transposed))]
        return _rand.choice(pool)
    else:
        # No previous voicing — pick randomly from library
        import random as _rand
        return _rand.choice(transposed)
DRUM_TYPES = ["Kick", "Snare", "Hi Hats", "Claps", "Rim"]


def _analyse_one_shot(filepath: Path) -> dict:
    """Extract timbral features from a single drum one-shot."""
    y, sr = librosa.load(str(filepath), sr=22050, mono=True, duration=2.0)
    if len(y) == 0:
    ...
    # Normalise: 500Hz = very warm, 8000Hz = very bright
    brightness = float(np.clip((centroid_mean - 500) / 7500, 0, 1))
    warmth = float(1.0 - brightness)

    # ── Onset strength → punch ────────────────────────────────────────────
    onset_env = librosa.onset.onset_strength(y=y, sr=sr)
    punch = float(np.clip(np.max(onset_env) / 20.0, 0, 1))

    # ── Attack time → ms from start to peak ──────────────────────────────
    peak_sample = int(np.argmax(np.abs(y)))
    attack_ms = float(peak_sample / sr * 1000)

    # ── Decay time → ms from peak to -20dB ───────────────────────────────
    peak_amp = np.max(np.abs(y))
    threshold = peak_amp * 0.1  # -20dB approx
    after_peak = np.abs(y[peak_sample:])
    below = np.where(after_peak < threshold)[0]
    if len(below) > 0:
        decay_ms = float(below[0] / sr * 1000)
    else:
        decay_ms = float(len(after_peak) / sr * 1000)

    # ── Roominess → ratio of tail energy to total energy ─────────────────
    total_energy = float(np.sum(y ** 2)) + 1e-9
    tail_start = min(peak_sample + int(0.05 * sr), len(y))  # 50ms after peak
    tail_energy = float(np.sum(y[tail_start:] ** 2))
    roominess = float(np.clip(tail_energy / total_energy, 0, 1))

    # ── Energy → RMS ──────────────────────────────────────────────────────
    rms = float(np.sqrt(np.mean(y ** 2)))
    energy = float(np.clip(rms / 0.3, 0, 1))

    # ── Vintage → spectral flatness (rough/noisy = vintage) ──────────────
    flatness = librosa.feature.spectral_flatness(y=y)[0]
    vintage = float(np.clip(np.mean(flatness) * 20, 0, 1))
...
async def tag_drums():
    """
    Scan Drums/ folder, analyse every WAV/OGG file, write Drums/metadata.json.
    Re-running updates the metadata — safe to call whenever new samples are added.
    """
    if not DRUMS_DIR.exists():
        raise HTTPException(status_code=404, detail="Drums/ folder not found in repo root")

    metadata = {}
    errors = {}

    for drum_type in DRUM_TYPES:
        type_dir = DRUMS_DIR / drum_type
        if not type_dir.exists():
            continue
        files = sorted(
            f for f in type_dir.iterdir()
            if f.suffix.lower() in (".wav", ".ogg", ".mp3", ".aiff", ".aif")
        )
        for f in files:
            rel_path = f"{drum_type}/{f.name}"
            try:
                features = _analyse_one_shot(f)
                features["type"] = drum_type
                metadata[rel_path] = features
            except Exception as e:
                errors[rel_path] = str(e)

    # Write metadata.json
    out_path = DRUMS_DIR / "metadata.json"
    out_path.write_text(json_module.dumps(metadata, indent=2))
...
class DrumMatchRequest(BaseModel):
    # Timbral targets from taste profile — all 0-1
    warmth: float = 0.6
    brightness: float = 0.4
    roominess: float = 0.3
    punch: float = 0.7
    vintage: float = 0.5
    pool_size: int = 5  # top N matches per slot returned for round-robin


@app.post("/match-drums")
async def match_drums(req: DrumMatchRequest):
    """
    Given a timbral target vector from the taste profile,
    return the top pool_size matches per drum slot (kick/snare/hat).
    Client shuffles and round-robins through the pool across exercises.
    """
    meta_path = DRUMS_DIR / "metadata.json"
    if not meta_path.exists():
        raise HTTPException(
            status_code=404,
            detail="Drums/metadata.json not found — run /tag-drums first"
        )

    metadata = json_module.loads(meta_path.read_text())

    # Target vector (only timbral dimensions we match on)
    target = np.array([
        req.warmth,
        req.brightness,
        req.roominess,
    ...
    for drum_type in DRUM_TYPES:
        candidates = {
            path: data for path, data in metadata.items()
            if data.get("type") == drum_type
        }
        if not candidates:
            result[drum_type] = []
            continue

        # Score each candidate by euclidean distance to target
        scored = []
        for path, data in candidates.items():
            vec = np.array([
                data.get("warmth", 0.5),
                data.get("brightness", 0.5),
                data.get("roominess", 0.5),
                data.get("punch", 0.5),
                data.get("vintage", 0.5),
            ])
            ...
            scored.append({
                ...
                "features": {k: data[k] for k in
                             ["warmth", "brightness", "punch", "roominess", "vintage", "energy"]
                             if k in data},
            })

        scored.sort(key=lambda x: x["distance"])
        result[drum_type] = scored[:req.pool_size]
...
@app.get("/drums/{drum_type}/{filename}")
async def serve_drum(drum_type: str, filename: str):
    """Serve a drum sample WAV file from the Drums/ folder."""
    from fastapi.responses import FileResponse
    safe_type = drum_type.replace('..', '').strip('/')
    safe_file = filename.replace('..', '').strip('/')
    sample_path = DRUMS_DIR / safe_type / safe_file
    if not sample_path.exists():
        raise HTTPException(status_code=404, detail=f"Sample not found: {drum_type}/{filename}")
...
async def exercise():
    exercise_file = _pathlib.Path(__file__).parent / 'exercise.html'
...
async def auto_tag_drums():
    """Auto-tag drum samples on startup if metadata.json is missing or stale."""
    if not DRUMS_DIR.exists():
        return
    meta_path = DRUMS_DIR / 'metadata.json'
    ...
    for drum_type in DRUM_TYPES:
        type_dir = DRUMS_DIR / drum_type
        if type_dir.exists():
            sample_files += [f for f in type_dir.iterdir()
                             if f.suffix.lower() in ('.wav', '.ogg', '.mp3', '.aiff', '.aif')]
    if not sample_files:
        return
    # Re-tag if metadata missing or sample count has changed
    needs_tag = not meta_path.exists()
    if not needs_tag:
        try:
            existing = json_module.loads(meta_path.read_text())
            needs_tag = len(existing) != len(sample_files)
        except Exception:
            needs_tag = True
    if needs_tag:
        print(f'[startup] Auto-tagging {len(sample_files)} drum samples...')
        metadata = {}
        for f in sample_files:
            drum_type = f.parent.name
            rel_path = f'{drum_type}/{f.name}'
            try:
                features = _analyse_one_shot(f)
                features['type'] = drum_type
                ...
                print(f'[startup] Failed to tag {rel_path}: {e}')
        meta_path.write_text(json_module.dumps(metadata, indent=2))
        print(f'[startup] Tagged {len(metadata)} samples -> {meta_path}')
    else:
        print(f'[startup] Drum metadata up to date ({len(sample_files)} samples)')
...
# Hardcoded profile — James's 4-song analysis
# Will be replaced by reading /app/profiles/profile.json once pipeline is wired
_HARDCODED_PROFILE = {
    "movements": [
        {"from": 6, "to": 4, "from_type": "min7", "to_type": "maj9", "label": "vi7 → IVmaj9"},
        {"from": 5, "to": 4, "from_type": "min7", "to_type": "maj7", "label": "v7 → IVmaj7"},
    ],
    "tempos": [80, 88, 96, 104],
    "keys": [
        {"root": 0,  "name": "C"},
    ...
    # Syncopation score 0.42, timing variance 26ms
    "syncopation": 0.42,
    "timing_variance_ms": 26.0,
    # Groove architecture — loose/human, moderate syncopation
    "groove": {
        "kick_syncopation": 0.45,   # how much kick deviates from 1+3
        "hat_density": 0.7,          # 0=sparse, 1=every 8th
        "ghost_notes": 0.3,          # probability of ghost hat hits
        "half_time_feel": False,
    },
    # Spectral warmth (1854Hz centroid → 0.7 warmth)
    "warmth": 0.7,
}

# State for cycling — persists across requests in memory
_exercise_state = {
    "movement_idx": 0,
    "key_idx": 0,
    "tempo_idx": 0,
...
DEGREE_TO_SEMITONE = {1: 0, 2: 2, 3: 4, 4: 5, 5: 7, 6: 9, 7: 11}

CHORD_TYPE_LABELS = {
    'maj7': 'maj7', 'maj9': 'maj9', 'min7': 'm7',
    'min9': 'm9',   'maj6': '6',    'dom7': '7',
}

CHORD_ROLES = {1: 'I', 2: 'II', 3: 'III', 4: 'IV', 5: 'V', 6: 'vi', 7: 'vii'}

# Left hand: root + 5th (low register, base MIDI 36 = C2)
# Right hand: extensions (mid register, base MIDI 60 = C4)
import random as _random

# Chord tones relative to root for each type
CHORD_TONES = {
    'maj7': [0, 4, 7, 11],       # root, 3, 5, maj7
    'maj9': [0, 4, 7, 11, 14],   # root, 3, 5, maj7, 9
    'min7': [0, 3, 7, 10],       # root, b3, 5, b7
    'min9': [0, 3, 7, 10, 14],   # root, b3, 5, b7, 9
    'maj6': [0, 4, 7, 9],        # root, 3, 5, 6
    'dom7': [0, 4, 7, 10],       # root, 3, 5, b7
}

# Essential tones (must include): 3rd and 7th define chord quality
ESSENTIAL_TONES = {
    'maj7': [4, 11],
    'maj9': [4, 11],
    'min7': [3, 10],
...
def _get_all_voicing_candidates(root_semitone: int, chord_type: str) -> list:
    """Get all valid note candidates for a chord in the rh register (C3=48 to B4=71)."""
    tones = CHORD_TONES.get(chord_type, CHORD_TONES['maj7'])
    candidates = []
    for tone in tones:
        # Generate all octaves of this tone that fall in C3-B4
        note = (root_semitone + tone) % 12
        for octave in range(3, 6):
            midi = note + (octave * 12)
            if 48 <= midi <= 71:
                candidates.append({'midi': midi, 'interval': tone % 12})
    return candidates


def _build_voicing(root_semitone: int, chord_type: str, prev_rh: list = None) -> list:
    """
    Build a voiced chord with proper register rules and voice leading.
    
    Left hand: root (C1) + 5th (nearby)
    Right hand: 3-4 notes in C3-B4, with voice leading from prev_rh if provided.
    Essential tones (3rd and 7th) always included.
    If prev_rh given, minimise total movement (smooth voice leading).
    """
    # Left hand — root at C1 register, 5th nearby
    lh_root = 24 + root_semitone   # C1 = 24
    while lh_root > 36: lh_root -= 12   # keep in C1-C2
    lh_fifth = lh_root + 7
    lh = [lh_root, lh_fifth]

    # Get all candidate notes for right hand
    candidates = _get_all_voicing_candidates(root_semitone, chord_type)
    essentials = ESSENTIAL_TONES.get(chord_type, [4, 7])
    
    # Must-include: one note for each essential interval
    essential_notes = []
    for ess_interval in essentials:
        ess_candidates = [c for c in candidates if c['interval'] == ess_interval % 12]
        if ess_candidates:
            if prev_rh:
                # Pick the one closest to previous voicing
                best = min(ess_candidates, key=lambda c: min(abs(c['midi'] - p) for p in prev_rh))
            else:
                # Pick middle of available range
                best = sorted(ess_candidates, key=lambda c: c['midi'])[len(ess_candidates)//2]
            essential_notes.append(best['midi'])

    # Fill remaining slots (target 4 notes total in rh)
    remaining_candidates = [c for c in candidates 
                            if c['midi'] not in essential_notes]
    
    # Score remaining by voice leading smoothness if prev_rh exists
    if prev_rh and remaining_candidates:
        remaining_candidates.sort(key=lambda c: min(abs(c['midi'] - p) for p in prev_rh))
    else:
        # No prev voicing — pick for spread/openness
        remaining_candidates.sort(key=lambda c: c['midi'])

    # Pick 1-2 more notes to make 3-4 total
    target_count = _random.choice([3, 4])
    extra_count = max(0, target_count - len(essential_notes))
    extra_notes = [c['midi'] for c in remaining_candidates[:extra_count]]
    
    rh = sorted(set(essential_notes + extra_notes))
    ...
    for n in rh:
        ...
        while n > 71: n -= 12
        rh_clean.append(n)
    rh_clean = sorted(set(rh_clean))

    return lh + rh_clean


def _derive_swing_ratio(timing_variance_ms: float, tempo: int) -> float:
    """
    Convert timing variance in ms to a swing ratio (0.5 = straight).
    26ms at 93 BPM = ~8% of an 8th note — barely perceptible.
    Clamp to 0.5–0.54 so nothing sounds like hip-hop swing.
    """
    eighth_note_ms = (60 / tempo) * 1000 / 2
    # Variance as proportion of 8th note — small numbers only
    ratio = timing_variance_ms / eighth_note_ms
    # Scale into a very tight range: 0.5 (straight) to 0.54 (barely human)
    swing = 0.5 + (ratio * 0.08)
    return round(min(max(swing, 0.5), 0.54), 4)


def _derive_chord_rhythm(syncopation: float) -> list:
    """
    Map syncopation score to chord beat positions in a 4-beat bar.
    0.0–0.2  → on the beat: [0, 2]
    0.2–0.4  → mild: [0, 1.5]  (anticipate beat 3 by an 8th)
    0.4–0.6  → moderate: [0, 1.5] with push feel
    0.6–1.0  → heavy: [0.5, 2.5] (both chords off the beat)
    """
    if syncopation < 0.2:
        return [0, 2]
...
def _derive_drum_pattern(groove: dict, syncopation: float) -> dict:
    """
    Generate a drum pattern from groove profile characteristics.
    Each call produces a different pattern that stays within the profile's
    feel — same syncopation level, hat density, and kick character,
    but specific hit positions are varied randomly within those constraints.
    """
    import random

    kick_sync   = groove.get('kick_syncopation', 0.3)
    hat_density = groove.get('hat_density', 0.7)
    half_time   = groove.get('half_time_feel', False)

    all_eighths = [round(i * 0.5, 1) for i in range(8)]  # 0..3.5

    # ── Snare ────────────────────────────────────────────────────────────────
    # Backbeat is the anchor — always on 1 and 3 (0-indexed beats)
    # Occasionally add a ghost hit based on syncopation
    snare = [1, 3] if not half_time else [2]
    if syncopation > 0.3 and random.random() < syncopation:
        # Add one ghost snare hit — pick a position not clashing with backbeat
        ghost_options = [b for b in all_eighths if b not in snare and b % 1 != 0]
        if ghost_options:
            snare = sorted(snare + [random.choice(ghost_options)])

    # ── Clap ─────────────────────────────────────────────────────────────────
    # Usually on beat 2 (index 1), sometimes doubled or shifted
    clap_options = [1, 1.5, 3]
    clap_count = 1 if syncopation < 0.5 else random.choice([1, 2])
    clap = sorted(random.sample(clap_options, min(clap_count, len(clap_options))))

    # ── Kick ─────────────────────────────────────────────────────────────────
    # Beat 0 is almost always the anchor kick
    # Additional hits drawn from a pool weighted by kick_syncopation
    kick_anchor = [0]

    # Pool of possible additional kick positions, weighted by syncopation
    if half_time:
        extra_pool = [2]
        max_extra = 1
    elif kick_sync < 0.25:
        ...
        extra_pool = [1.5, 2, 2.5, 3]
        max_extra = 2
    else:
        extra_pool = [0.5, 1.5, 2, 2.5, 3, 3.5]
        max_extra = 3

    # Pick n extra hits, avoiding clashes with snare backbeat
    n_extra = random.randint(1, max_extra)
    available = [b for b in extra_pool if b not in snare]
    extra_kicks = random.sample(available, min(n_extra, len(available)))
    kick = sorted(kick_anchor + extra_kicks)

    # ── Hi hats ──────────────────────────────────────────────────────────────
    # Base: on-beat quarter notes always present
    # Off-beats added based on hat_density, with some random dropout
    on_beats = [0.0, 1.0, 2.0, 3.0]
    off_beats = [0.5, 1.5, 2.5, 3.5]

    hat = list(on_beats)
    for b in off_beats:
        # Probability of including each off-beat scales with density
        # Add some randomness so pattern varies each exercise
        if random.random() < hat_density * random.uniform(0.6, 1.0):
            hat.append(b)
...
async def generate_exercise():
    """
    Generate a complete exercise spec derived from the taste profile.
    Cycles through movements, keys, and tempos on each call.
    """
    profile = _HARDCODED_PROFILE

    # Pull current cycle state
    mov_idx   = _exercise_state['movement_idx']
    key_idx   = _exercise_state['key_idx']
    tempo_idx = _exercise_state['tempo_idx']

    movement = profile['movements'][mov_idx % len(profile['movements'])]
    key      = profile['keys'][key_idx % len(profile['keys'])]
    tempo    = profile['tempos'][tempo_idx % len(profile['tempos'])]

    # Advance state for next call
    _exercise_state['movement_idx'] += 1
    # Key changes every 2 exercises (one full movement cycle)
    if _exercise_state['movement_idx'] % len(profile['movements']) == 0:
        _exercise_state['key_idx'] += 1
    # Tempo changes every 4 exercises
    if _exercise_state['movement_idx'] % 4 == 0:
        _exercise_state['tempo_idx'] += 1

    # Build chord voicings from library with voice leading between chords
    chords = []
    prev_rh = None
    for degree, chord_type in [
        (movement['from'], movement['from_type']),
        (movement['to'],   movement['to_type']),
    ]:
        semitone = (key['root'] + DEGREE_TO_SEMITONE[degree]) % 12
        name = NOTE_NAMES[semitone] + CHORD_TYPE_LABELS.get(chord_type, chord_type)
        voicing = _pick_voicing(chord_type, semitone, prev_rh_midi=prev_rh)
        prev_rh = voicing['rh_midi']
        chords.append({
            'name':   name,
            'role':   CHORD_ROLES.get(degree, str(degree)),
            'notes':  voicing['all_midi'],
            'voicing_id': voicing['id'],
        })

    # Derive musical parameters from profile
    swing_ratio   = _derive_swing_ratio(profile['timing_variance_ms'], tempo)
    chord_rhythm  = _derive_chord_rhythm(profile['syncopation'])
    drum_pattern  = _derive_drum_pattern(profile['groove'], profile['syncopation'])
...
# ─────────────────────────────────────────────────────────────────────────────
# Voicings library — parse MIDI files and build index
# ─────────────────────────────────────────────────────────────────────────────
...
def _pick_voicing(chord_type: str, root_semitone: int, prev_rh_midi: list = None) -> dict:
    ...
    if prev_rh_midi:
        ...
        def vl_score(v):
            ...
            return total
        transposed.sort(key=vl_score)
        # Pick randomly from voicings within 1 semitone of the best score.
        # Gives variety when multiple voicings are equally smooth, but never
        # pulls in a noticeably worse option.
        import random as _rand
        best_score = vl_score(transposed[0])
        pool = [v for v in transposed if vl_score(v) <= best_score + 1]
        return _rand.choice(pool)
    else:
        # No previous voicing — pick randomly from library
        import random as _rand
...
def _midi_note_to_name(midi: int) -> str:
    names = ['C', 'C#', 'D', 'Eb', 'E', 'F', 'F#', 'G', 'Ab', 'A', 'Bb', 'B']
    return names[midi % 12] + str(midi // 12 - 1)


def _parse_voicing_midi(path: Path) -> list:
    """Extract note MIDI numbers from a voicing MIDI file."""
    try:
        import mido
        ...
        print(f'[voicings] Failed to parse {path.name}: {e}')
        return []


def _spread_score(notes: list) -> float:
    """Semitone span of notes (RH only)."""
    if len(notes) < 2:
        return 0.0
    return float(max(notes) - min(notes))


def _spread_label(semitones: float) -> str:
    if semitones <= 12:
        return 'close'
    elif semitones <= 19:
        return 'mid'
...
@app.post('/parse-voicings')
async def parse_voicings():
    """
    Scan library/voicings/ folder, parse every MIDI pair (RH + LH),
    calculate spread scores and note lists, write voicings_index.json.
    Run once after uploading new MIDI files.
    """
    if not LIBRARY_DIR.exists():
        raise HTTPException(
            status_code=404,
            detail='library/voicings/ folder not found — create it in the repo root'
        )

    # Find all RH files — each implies a matching LH
    rh_files = sorted(LIBRARY_DIR.glob('*_RH.mid'))
    if not rh_files:
        return {'indexed': 0, 'message': 'No *_RH.mid files found in library/voicings/'}

    voicings = []
    errors = []

    for rh_path in rh_files:
        stem = rh_path.stem.replace('_RH', '')   # e.g. maj7_close_01
        parts = stem.split('_')
        if len(parts) < 3:
            errors.append(f'Bad filename format: {rh_path.name} — expected chord_style_index_RH.mid')
            continue

        chord_type = parts[0]
        voicing_style = parts[1]
        index = parts[2]

        lh_path = LIBRARY_DIR / f'{stem}_LH.mid'

        rh_notes = _parse_voicing_midi(rh_path)
        lh_notes = _parse_voicing_midi(lh_path) if lh_path.exists() else []

        spread = _spread_score(rh_notes)
        ...
        voicings.append({
            ...
            'index': index,
            'spread_score': round(spread, 1),
            'spread_label': spread_label,
            'notes_rh': rh_notes,
            'notes_lh': lh_notes,
            'note_names_rh': [_midi_note_to_name(n) for n in rh_notes],
            'note_names_lh': [_midi_note_to_name(n) for n in lh_notes],
            'files': {
                'RH': rh_path.name,
                'LH': lh_path.name if lh_path.exists() else None,
            }
        })
    ...
    index_data = {
        'version': '1.0',
        'total': len(voicings),
        'chord_types': sorted(set(v['chord_type'] for v in voicings)),
        'voicing_styles': sorted(set(v['voicing_style'] for v in voicings)),
        'voicings': voicings,
    }

    index_path = LIBRARY_DIR.parent / 'voicings_index.json'
    index_path.write_text(json_module.dumps(index_data, indent=2))

    return {
        'indexed': len(voicings),
        'errors': errors,
        'index_path': str(index_path),
        'summary': {
            chord: [v['id'] for v in voicings if v['chord_type'] == chord]
            for chord in index_data['chord_types']
        }
    }


@app.get('/voicings')
async def get_voicings(chord_type: str = None, voicing_style: str = None):
    """
    Return the voicings index, optionally filtered by chord_type and/or voicing_style.
    Used by the exercise engine to pick the right voicing for a given profile.
    """
    index_path = LIBRARY_DIR.parent / 'voicings_index.json'
    if not index_path.exists():
        raise HTTPException(
            status_code=404,
            detail='voicings_index.json not found — run POST /parse-voicings first'
        )

    data = json_module.loads(index_path.read_text())
    voicings = data['voicings']

    if chord_type:
        voicings = [v for v in voicings if v['chord_type'] == chord_type]
    if voicing_style:
        voicings = [v for v in voicings if v['voicing_style'] == voicing_style]

    return {'total': len(voicings), 'voicings': voicings}


@app.get('/library/voicings/{filename}')
async def serve_voicing_midi(filename: str):
    """Serve a voicing MIDI file from the library."""
    from fastapi.responses import FileResponse
    safe_name = filename.replace('..', '').strip('/')
    midi_path = LIBRARY_DIR / safe_name
    if not midi_path.exists():
        raise HTTPException(status_code=404, detail=f'Voicing file not found: {filename}')
