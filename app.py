...
def build_midi_from_analysis(wav_path: str, song_id: str, analysis: dict) -> str:
    ...
    Tracks:
    1. Drums — kick, snare, hi-hat from onset detection
    ...
    # Re-detect drum onsets from the stem if available
    stem_dir = str(WORK_DIR / f"{song_id}_stems")
    ...
    # Beat timing (needed by drums, bass, and chords)
    ...
    try:
        ...
        # Try to find drum stem
        stems = {}
        for model_name in ["ensemble", "htdemucs_ft", "mdx_extra", "htdemucs"]:
            stem_check = Path(stem_dir) / model_name / Path(wav_path).stem
            if stem_check.exists():
                for sn in ["drums", "bass", "vocals", "other"]:
                    sp = stem_check / f"{sn}.wav"
                    if sp.exists():
                        stems[sn] = str(sp)
                if stems:
                    break
        ...
        if "drums" in stems:
            y_drums, sr_d = librosa.load(stems["drums"], sr=44100, mono=True)
            nyq = sr_d / 2

            # Build the 16th note grid
            sixteenth_sec = beat_duration / 4.0
            track_duration = len(y_drums) / sr_d
            grid_16ths = np.arange(0, track_duration, sixteenth_sec)
            n_bars = int(n_beats / beats_per_bar) + 1
            sixteenths_per_bar = beats_per_bar * 4

            def detect_with_velocity(y_filtered, sr, noise_floor_pct=15):
                """
                Detect onsets and return (time, strength) pairs.
                
                Uses onset strength as a proxy for transient energy / velocity.
                Removes anything below the noise floor (bottom N percentile).
                Scales remaining strengths to MIDI velocity range 30-127.
                """
                onset_env = librosa.onset.onset_strength(y=y_filtered, sr=sr)
                onset_frames = librosa.onset.onset_detect(
                    y=y_filtered, sr=sr, onset_envelope=onset_env
                )
                ...
                for frame in onset_frames:
                    ...
                    hits.append((time, strength))

                if not hits:
                    return []

                # Find noise floor and remove bleed/noise
                all_strengths = [s for _, s in hits]
                noise_threshold = np.percentile(all_strengths, noise_floor_pct)
                max_strength = max(all_strengths)

                if max_strength <= 0:
                    return []

                # Filter out noise and scale to MIDI velocity
                result = []
                for time, strength in hits:
                    if strength <= noise_threshold:
                        continue  # Below noise floor — bleed or artifact

                    # Scale: noise_threshold→max maps to velocity 30→127
                    normalised = (strength - noise_threshold) / (max_strength - noise_threshold)
                    velocity = int(30 + normalised * 97)
                    velocity = min(127, max(30, velocity))
                    result.append((time, velocity))

                return result

            def snap_hits_to_grid(hits, grid, tolerance_fraction=0.45):
                """
                Snap (time, velocity) pairs to nearest 16th grid position.
                If multiple hits snap to the same grid point, keep the loudest.
                Returns dict of {grid_index: velocity}.
                """
                grid_hits = {}
                for time, vel in hits:
                    diffs = np.abs(grid - time)
                    nearest_idx = int(np.argmin(diffs))
                    if diffs[nearest_idx] < sixteenth_sec * tolerance_fraction:
                        # Keep the loudest hit at each grid position
                        if nearest_idx not in grid_hits or vel > grid_hits[nearest_idx]:
                            grid_hits[nearest_idx] = vel
                return grid_hits

            def find_base_pattern(grid_hits, sixteenths_per_bar, n_bars, min_consistency=0.4):
                """
                Find which grid positions within a bar are hit consistently.
                
                A position is 'consistent' if it has a hit in at least min_consistency
                fraction of bars. Returns set of bar-relative positions (0 to sixteenths_per_bar-1).
                """
                # Count how many bars have a hit at each bar-relative position
                position_counts = {}
                for grid_idx in grid_hits:
                    bar_position = grid_idx % sixteenths_per_bar
                    if bar_position not in position_counts:
                        position_counts[bar_position] = 0
                    position_counts[bar_position] += 1

                # Positions hit in enough bars are part of the base pattern
                base_positions = set()
                for pos, count in position_counts.items():
                    if count >= n_bars * min_consistency:
                        base_positions.add(pos)

                return base_positions

            def classify_hits(grid_hits, base_pattern, sixteenths_per_bar):
                """
                Classify each hit as:
                - 'pattern': matches the base pattern (consistent across bars)
                - 'variation': doesn't match pattern but has strong velocity
                - 'weak': doesn't match pattern and has low velocity (likely noise)
                
                Returns filtered dict of {grid_index: velocity} with weak hits removed.
                """
                if not grid_hits:
                    return {}

                all_vels = list(grid_hits.values())
                median_vel = np.median(all_vels)

                filtered = {}
                for grid_idx, vel in grid_hits.items():
                    bar_position = grid_idx % sixteenths_per_bar

                    if bar_position in base_pattern:
                        # Part of the consistent pattern — keep it
                        filtered[grid_idx] = vel
                    elif vel >= median_vel * 0.7:
                        # Strong hit outside the pattern — intentional variation/fill
                        filtered[grid_idx] = vel
                    # else: weak hit outside pattern — drop it (noise/bleed)

                return filtered

            def grid_idx_to_tick(idx):
                return idx * ticks_per_16th

            # ── Kick ───────────────────────────────────────────────
            sos_k = butter(4, [20 / nyq, 150 / nyq], btype='band', output='sos')
            y_kick = sosfilt(sos_k, y_drums)

            kick_hits = detect_with_velocity(y_kick, sr_d, noise_floor_pct=20)
            kick_grid = snap_hits_to_grid(kick_hits, grid_16ths)
            kick_pattern = find_base_pattern(kick_grid, sixteenths_per_bar, n_bars, min_consistency=0.35)
            kick_final = classify_hits(kick_grid, kick_pattern, sixteenths_per_bar)

            for idx, vel in kick_final.items():
                drum_events.append((grid_idx_to_tick(idx), KICK_NOTE, vel))

            # ── Snare ──────────────────────────────────────────────
            sos_s = butter(4, [200 / nyq, 1000 / nyq], btype='band', output='sos')
            y_snare = sosfilt(sos_s, y_drums)

            # Higher noise floor for snare — more susceptible to kick/hat bleed
            snare_hits = detect_with_velocity(y_snare, sr_d, noise_floor_pct=35)
            snare_grid = snap_hits_to_grid(snare_hits, grid_16ths)
            snare_pattern = find_base_pattern(snare_grid, sixteenths_per_bar, n_bars, min_consistency=0.4)
            snare_final = classify_hits(snare_grid, snare_pattern, sixteenths_per_bar)

            for idx, vel in snare_final.items():
                drum_events.append((grid_idx_to_tick(idx), SNARE_NOTE, vel))

            # ── Hi-hat ─────────────────────────────────────────────
            sos_h = butter(4, 6000 / nyq, btype='high', output='sos')
            y_hat = sosfilt(sos_h, y_drums)

            hat_hits = detect_with_velocity(y_hat, sr_d, noise_floor_pct=15)
            hat_grid = snap_hits_to_grid(hat_hits, grid_16ths)
            hat_pattern = find_base_pattern(hat_grid, sixteenths_per_bar, n_bars, min_consistency=0.3)
            hat_final = classify_hits(hat_grid, hat_pattern, sixteenths_per_bar)

            for idx, vel in hat_final.items():
                drum_events.append((grid_idx_to_tick(idx), HAT_CLOSED, vel))

    except Exception as e:
        pass  # No drums if stems unavailable
