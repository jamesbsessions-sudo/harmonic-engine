...
def _run_basic_pitch(audio_path, onset_threshold=0.5, frame_threshold=0.3,
    ...
    """Run Basic Pitch with given params, return sorted note events or None."""
    try:
        ...
        if note_events:
            return sorted(note_events, key=lambda x: x[0])
    except Exception:
        pass
    return None


def _note_events_to_sequence(note_events):
    """
    Convert Basic Pitch note events to a melody sequence.
    Merges consecutive duplicate notes (same pitch back to back = one held note).
    ...
    for event in note_events:
        ...
        raw_sequence.append(midi_note)
        raw_times.append(start_time)

    # ── Merge consecutive duplicates ───────────────────────────
    # If the same pitch appears back-to-back, it's a held note that
    # Basic Pitch split into separate events — merge into one.
    melody_sequence = []
    timestamped_pitches = []

    for i, midi_note in enumerate(raw_sequence):
        if melody_sequence and midi_note == melody_sequence[-1]:
            continue  # skip duplicate
        melody_sequence.append(midi_note)
        hz = 440.0 * (2.0 ** ((midi_note - 69) / 12.0))
        timestamped_pitches.append((raw_times[i], hz))
...
def extract_melody(audio: np.ndarray, sr: int = 44100, audio_path: str = None):
    """
    Extract vocal melody using Spotify's Basic Pitch (neural note transcription).
    
    Uses a two-pass hybrid approach:
    1. Strict pass (onset=0.6, frame=0.3) — high-confidence backbone notes
    2. Sensitive pass (onset=0.5, frame=0.2) — catches quieter/shorter notes
    Then filters the sensitive pass to remove hallucinations (notes outside
    the vocal range established by the strict pass) and merges the two sets.
    
    Falls back to pyin if Basic Pitch isn't available.
    """
    ...
    # ── Pass 1: Strict — high-confidence backbone ─────────────
    strict_events = None
    if resolve_path:
        strict_events = _run_basic_pitch(
            resolve_path,
            onset_threshold=0.6,
            frame_threshold=0.3,
    ...
    # ── Pass 2: Sensitive — catches quieter notes ─────────────
    sensitive_events = None
    if resolve_path:
        sensitive_events = _run_basic_pitch(
            resolve_path,
            onset_threshold=0.5,
            frame_threshold=0.2,
            minimum_note_length=58,
    ...
    # ── Fallback to pyin ───────────────────────────────────────
    if not strict_events and not sensitive_events:
        return _extract_melody_pyin(audio, sr)

    # ── Merge: use strict as backbone, fill gaps from sensitive ─
    if strict_events and sensitive_events:
        # Establish vocal range from the strict (confident) detections
        strict_midis = [int(e[2]) for e in strict_events]
        range_low = min(strict_midis) - 2   # allow 2 semitones below lowest confident note
        range_high = max(strict_midis) + 2  # allow 2 semitones above highest confident note

        # Filter sensitive events: remove anything outside the confident range
        filtered_sensitive = [
            e for e in sensitive_events
            if range_low <= int(e[2]) <= range_high
        ]

        # Build a timeline from strict events — find gaps where sensitive
        # events can fill in. A "gap" is any time region not covered by
        # a strict event.
        strict_coverage = set()
        for e in strict_events:
            # Mark time slots covered by strict events (10ms resolution)
            start_ms = int(float(e[0]) * 100)
            end_ms = int(float(e[1]) * 100)
            for t in range(start_ms, end_ms + 1):
                strict_coverage.add(t)

        # Add sensitive events that fall in gaps
        gap_fills = []
        for e in filtered_sensitive:
            start_ms = int(float(e[0]) * 100)
            mid_ms = int((float(e[0]) + float(e[1])) / 2 * 100)
            # If the midpoint of this event isn't covered by a strict event,
            # it's in a gap — include it
            if mid_ms not in strict_coverage:
                gap_fills.append(e)

        # Merge and sort by time
        merged_events = sorted(strict_events + gap_fills, key=lambda x: x[0])
        note_events = merged_events

    elif strict_events:
        note_events = strict_events
    else:
        # Only sensitive available — filter by median range
        midis = [int(e[2]) for e in sensitive_events]
        median_midi = sorted(midis)[len(midis) // 2]
        note_events = [
            e for e in sensitive_events
            if abs(int(e[2]) - median_midi) <= 14  # within ~octave of median
        ]

    # ── Convert to our format with duplicate merging ───────────
    melody_sequence, timestamped_pitches = _note_events_to_sequence(note_events)
