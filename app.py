...
def detect_bass_roots(bass_path: str, beat_frames, sr=44100):
    ...
    # If a bass note appears for only 1 beat and is a perfect 5th,
    # major 3rd, or minor 7th above the surrounding sustained note,
    # it's almost certainly an overtone resonance — replace with
    # the sustained note. This avoids changing onset detection
    # parameters which affects chord detection downstream.
    OVERTONE_INTERVALS = {7, 4, 10, 3, 5}  # p5, M3, m7, m3, p4 in semitones
    ...
    for i in range(1, len(cleaned_bass) - 1):
        ...
        if curr != prev and curr != nxt:
            # Single-beat blip — check if overtone of either neighbour
            if prev == nxt and is_overtone_of(curr, prev):
                cleaned_bass[i] = prev
            elif is_overtone_of(curr, prev):
                cleaned_bass[i] = prev
            elif is_overtone_of(curr, nxt):
                cleaned_bass[i] = nxt
...
def extract_melody(audio: np.ndarray, sr: int = 44100):
    ...
    # each frame ≈ 6ms. Kernel=21 ≈ 122ms — similar window to the
    # old 11-frame kernel at hop=512.
    if len(raw_midi) > 21:
        smoothed_midi = medfilt(raw_midi, kernel_size=21)
    elif len(raw_midi) > 11:
