...
def detect_bass_roots(bass_path: str, beat_frames, sr=44100):
    ...
    # ── Primary: pyin fundamental tracking ─────────────────────
    # More accurate than chroma on clean stems because it tracks
    # the actual f0 rather than picking the loudest chroma bin
    # (which can be an overtone).
    ...
    # ── Fallback: chroma for beats where pyin finds nothing ────
    ...
    for i in range(len(beat_frames_list)):
        ...
        # Try pyin first — use any frame with voicing probability > 0.3
        ...
        if len(confident) >= 2:
            # pyin found a pitch — use the median
        ...
        # pyin failed — fall back to chroma
        if i < beat_chroma.shape[1]:
            frame = beat_chroma[:, i]
            if frame.sum() > 0:
