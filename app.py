...
def extract_melody(audio: np.ndarray, sr: int = 44100):
    ...
    # Apply median filter (window of 5 frames = 50ms at 100fps)
    if len(raw_midi) > 5:
...
def analyse_audio(wav_path: str, song_id: str) -> dict:
    ...
    try:
        ...
        # ── Melody extraction (RMVPE on full mix — no vocal stem needed) ──
        # RMVPE extracts vocal pitch directly from polyphonic audio,
        # eliminating instrument bleed that contaminated torchcrepe results.
        try:
