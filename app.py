...
def build_midi_from_analysis(wav_path: str, song_id: str, analysis: dict) -> str:
    ...
    # Beat timing (needed by bass and chords below)
    ...
    try:
        ...
        if drum_stem_path and os.path.exists(drum_stem_path):
            ...
            print(f"[DRUMS] 16th note = {sixteenth_sec:.4f}s, beat = {60.0/tempo_bpm:.4f}s, BPM = {tempo_bpm}")

            # Quantise each hit to the nearest 16th note grid position
            for note in all_notes:
                grid_idx = round(note.start / sixteenth_sec)
