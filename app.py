...
def detect_bass_roots(bass_path: str, beat_frames, sr=44100):
    ...
    onset_frames_detected = librosa.onset.onset_detect(
        ...
        backtrack=True,
        delta=0.12,   # Raise from default 0.07 — filters overtone ghost onsets on sustaining beats
        wait=4,       # Min 4 frames (~50ms) between onsets — prevents rapid re-triggering
    )
    ...
    for i in range(beat_chroma.shape[1]):
        ...
        has_onset = any(
            abs(beat_time - ot) < beat_duration * 0.35  # Tightened from 0.6 — only count onsets close to the beat downstroke
...
def extract_melody(audio: np.ndarray, sr: int = 44100):
    ...
    f0, voiced_flag, voiced_probs = librosa.pyin(
        audio, sr=sr,
        fmin=librosa.note_to_hz('G2'),   # Raised from C2 (65Hz) — C2 allows sub-vocal harmonics as candidates, causing octave errors
    ...
    # Wider median filter (11 frames ≈ 130ms at pyin's default hop)
    if len(raw_midi) > 11:
    ...
    # ── Remove short notes (< 4 frames ≈ 50ms) ────────────────
    min_frames = 4
    filtered_runs = []
