...
def extract_melody(vocal_path: str, sr=44100):
    ...
    Returns detailed melody analysis with improved accuracy.
    """
    from scipy.signal import butter, sosfilt

    y_vocal, sr_loaded = librosa.load(vocal_path, sr=16000, mono=True)

    # ── Bandpass filter the vocal stem to isolate voice fundamentals ──
    # Human voice fundamentals: ~150Hz (low male) to ~800Hz (high female)
    # This removes instrument bleed in low bass and high harmonic ranges
    nyquist = 16000 / 2
    sos_vocal = butter(4, [150 / nyquist, 800 / nyquist], btype='band', output='sos')
    y_filtered = sosfilt(sos_vocal, y_vocal).astype(np.float32)

    audio_tensor = torch.from_numpy(y_filtered).unsqueeze(0).float()

    # Run CREPE pitch detection — 'full' model with Viterbi decoding
    # Viterbi penalises large pitch jumps, reducing spurious note detection
    pitch, periodicity = torchcrepe.predict(
        audio_tensor,
        sample_rate=16000,
        hop_length=160,  # 10ms at 16kHz
        fmin=150,        # Match our bandpass filter
        fmax=800,        # Match our bandpass filter
        model='full',
    ...
    # ── High confidence threshold to reject instrument bleed ───
    voiced_mask = periodicity > 0.75
    ...
    # ── Convert to MIDI and apply median smoothing ─────────────
    # Median smoothing over a small window removes frame-level jitter
    # without losing real note changes
    raw_midi = np.array([hz_to_midi(hz) for hz in voiced_pitches])
    raw_midi = raw_midi[raw_midi != None].astype(float)

    # Apply median filter (window of 5 frames = 50ms)
    if len(raw_midi) > 5:
    ...
    # This removes the "held note" frames but keeps each distinct pitch
    melody_sequence = []
    for note in quantised:
        if not melody_sequence or note != melody_sequence[-1]:
            melody_sequence.append(int(note))

    # ── Remove isolated single-frame notes (likely artefacts) ──
    # If a note appears only once between two identical notes, it's noise
    if len(melody_sequence) > 2:
        ...
        for i in range(1, len(melody_sequence) - 1):
            ...
            # Keep if it's not a single-frame blip between identical notes
    ...
    # Output all distinct notes (not capped at 32)
    melody_notes = []
