...
def detect_bass_roots(bass_path: str, beat_frames, sr=44100):
    ...
    # Use pyin to track the actual fundamental pitch of the bass stem.
    # This is far more accurate than chroma on a clean stem because
    # chroma folds octaves and picks up overtones as root notes.
    ...
    # Convert f0 to note names per frame
    hop_length = 512  # pyin default
    n_frames = len(f0)

    # For each beat, find the dominant pitch
    bass_notes = []
    ...
    for i in range(len(beat_frames_list)):
        ...
        # Get all voiced pitches in this beat window
        beat_f0 = f0[start_frame:end_frame]
        beat_voiced = voiced_flag[start_frame:end_frame]

        # Only consider frames where pyin is confident there's a pitch
        voiced_pitches = beat_f0[beat_voiced]

        if len(voiced_pitches) == 0:
            bass_notes.append(None)
            continue

        # Take the median pitch (robust to outliers from transients)
        ...
        # Convert to note name (just the pitch class, no octave)
        note_name = librosa.hz_to_note(median_hz)
        # Strip octave number to get just the root
        root = note_name.rstrip('0123456789')
        # Normalise sharp/flat naming
