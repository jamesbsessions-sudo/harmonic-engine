...
def build_midi_from_analysis(wav_path: str, song_id: str, analysis: dict) -> str:
    ...
    try:
        ...
        if drum_stem_path and os.path.exists(drum_stem_path):
            ...
            # What the 16th grid expects at 94 BPM
            sixteenth_sec = 60.0 / tempo_bpm / 4.0
            print(f"[DRUMS] 16th note = {sixteenth_sec:.4f}s, beat = {60.0/tempo_bpm:.4f}s, BPM = {tempo_bpm}, audio_offset = {audio_offset:.4f}s")

            # Quantise each hit to the nearest 16th note grid position
            # Offset from audio_offset so beat 1 = tick 0
            for note in all_notes:
                relative_time = note.start - audio_offset
                if relative_time < -sixteenth_sec:
                    continue  # Before beat 1 — skip
                grid_idx = round(relative_time / sixteenth_sec)
                grid_idx = max(0, grid_idx)
                snapped_tick = grid_idx * ticks_per_16th
                drum_events.append((snapped_tick, note.pitch, note.velocity))
