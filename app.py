...
def _build_voicing(root_semitone: int, chord_type: str) -> list:
    ...
    rh = [48 + root_semitone + i for i in shape['rh']]
    return lh + rh
...
def _derive_swing_ratio(timing_variance_ms: float, tempo: int) -> float:
    ...
    # Variance as proportion of 8th note, scaled to swing range
    swing = 0.5 + (timing_variance_ms / (eighth_note_ms * 2))
    return round(min(max(swing, 0.5), 0.65), 4)
