...
def build_midi_from_analysis(wav_path: str, song_id: str, analysis: dict) -> str:
    ...
    try:
        # Check for stems from the analysis — they may have been cleaned up
        # So we re-run onset detection on the original audio
        ...
        if not stems:
            # Re-run separation for MIDI export
            try:
                stems = separate_full_stems(wav_path, stem_dir)
            except Exception:
                stems = {}
        ...
        if "drums" in stems:
            ...
            for t in kick_times:
                drum_events.append((seconds_to_ticks(t), KICK_NOTE, 100))

            # Snare
            sos_s = butter(4, [200 / nyq, 1000 / nyq], btype='band', output='sos')
            y_snare = sosfilt(sos_s, y_drums)
            snare_times = librosa.onset.onset_detect(y=y_snare, sr=sr_d, units='time')
            # Filter to strong hits only
            ...
            if len(snare_frames) > 0:
                strengths = [snare_env[f] if f < len(snare_env) else 0 for f in snare_frames]
                if strengths:
                    thresh = np.percentile(strengths, 60)
                    strong = [f for f, s in zip(snare_frames, strengths) if s >= thresh]
                    snare_times = librosa.frames_to_time(np.array(strong), sr=sr_d)
            for t in snare_times:
                drum_events.append((seconds_to_ticks(t), SNARE_NOTE, 100))
            ...
            for t in hat_times:
                drum_events.append((seconds_to_ticks(t), HAT_CLOSED, 80))
    ...
    duration = librosa.get_duration(filename=wav_path) if os.path.exists(wav_path) else 30.0
