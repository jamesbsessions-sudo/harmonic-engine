...
VOICING_SHAPES = {
    'maj7': {'lh': [0, 7],  'rh': [4, 11, 16]},
    'maj9': {'lh': [0, 7],  'rh': [4, 11, 14]},
    'min7': {'lh': [0, 7],  'rh': [3, 10, 15]},
    'min9': {'lh': [0, 7],  'rh': [3, 10, 14]},
    'maj6': {'lh': [0, 7],  'rh': [4, 9,  16]},
    'dom7': {'lh': [0, 7],  'rh': [4, 10, 14]},
}


def _build_voicing(root_semitone: int, chord_type: str) -> list:
    shape = VOICING_SHAPES.get(chord_type, VOICING_SHAPES['maj7'])
    # Left hand: root + 5th in C1-C2 register (MIDI 24-36)
    lh = [24 + root_semitone + i for i in shape['lh']]
    # Right hand: extensions starting at C3 register (MIDI 48)
    rh_raw = [48 + root_semitone + i for i in shape['rh']]
    # Enforce register: all rh notes must be C3 (48) to B4 (71)
    rh = []
    for n in rh_raw:
        while n < 48:   # below C3 — raise an octave
            n += 12
        while n > 71:   # above B4 — drop an octave
            n -= 12
        rh.append(n)
...
async def generate_exercise():
    ...
    if _exercise_state['key_idx'] % len(profile['keys']) == 0:
        _exercise_state['tempo_idx'] += 1

    # Build chord voicings
    chords = []
    for degree, chord_type, role_num in [
        (movement['from'], movement['from_type'], movement['from']),
        (movement['to'],   movement['to_type'],   movement['to']),
    ]:
        ...
        chords.append({
            ...
            'notes': _build_voicing(semitone, chord_type),
        })
