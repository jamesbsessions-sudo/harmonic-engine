...
def detect_bass_roots(bass_path: str, beat_frames, sr=44100):
    ...
    # ── Onset detection to know when a NEW note is played ──────
    # Only update the bass note when there's an actual onset (new attack).
    ...
    onset_times = set(librosa.frames_to_time(onset_frames, sr=sr))

    n_frames = len(f0)
    beat_frames_list = list(beat_frames)
    beat_times_local = librosa.frames_to_time(beat_frames, sr=sr)
    bass_notes = []
    last_good_note = None

    for i in range(len(beat_frames_list)):
        start_frame = beat_frames_list[i]
        end_frame = beat_frames_list[i + 1] if i + 1 < len(beat_frames_list) else n_frames
        beat_time = float(beat_times_local[i]) if i < len(beat_times_local) else 0

        # Check if there's an onset near this beat (within half a beat)
        beat_duration = 0.5  # rough default
        ...
        # Try pyin — use frames with voicing probability > 0.3
        beat_f0 = f0[start_frame:end_frame]
        beat_probs = voiced_probs[start_frame:end_frame]

        confident = beat_f0[beat_probs > 0.3]
        confident = confident[~np.isnan(confident)]

        detected_note = None
        if len(confident) >= 2:
            median_hz = float(np.median(confident))
            if median_hz > 0:
                note_name = librosa.hz_to_note(median_hz)
                root = note_name.rstrip('0123456789')
                if root in NOTE_TO_NUM:
                    detected_note = root

        if has_onset and detected_note:
            # New note articulation with a confident pitch — use it
            last_good_note = detected_note
            bass_notes.append(detected_note)
        elif has_onset and not detected_note:
            # Onset but pyin couldn't get pitch — could be a percussive hit
            # or a very low note. Keep previous note as best guess.
            bass_notes.append(last_good_note)
        elif detected_note and detected_note != last_good_note:
            # No onset but pitch changed — might be a legato note change
            last_good_note = detected_note
            bass_notes.append(detected_note)
        else:
            # No onset, same or no pitch — bass is sustaining. Carry forward.
            bass_notes.append(last_good_note)
