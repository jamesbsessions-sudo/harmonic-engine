...
def extract_melody(vocal_path: str, sr=44100):
    ...
    # Run CREPE pitch detection — use 'full' model for better accuracy
    pitch, periodicity = torchcrepe.predict(
        audio_tensor,
        sample_rate=16000,
        hop_length=160,  # 10ms at 16kHz
        fmin=80,         # ~E2 (low male vocal)
        fmax=1200,       # ~D6 (high female vocal)
        model='full',    # Best accuracy
        device='cpu',
    ...
    # ── Higher confidence threshold to reject instrument bleed ──
    # 0.6 is stricter than 0.5, reduces false positives from bleed
    voiced_mask = periodicity > 0.6

    voiced_pitches = pitch[voiced_mask]
    voiced_periodicities = periodicity[voiced_mask]
