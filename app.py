...
def extract_melody(audio: np.ndarray, sr: int = 44100, audio_path: str = None):
    ...
    # ── Try Basic Pitch if we have a file path ─────────────────
    if audio_path is not None:
        try:
            ...
            _model_output, _midi_data, note_events = bp_predict(
                audio_path,
                minimum_frequency=130.0,    # C3 — realistic vocal floor
                maximum_frequency=1000.0,   # B5 — realistic vocal ceiling
                onset_threshold=0.5,
                frame_threshold=0.3,
                minimum_note_length=80,     # ms — allow shorter notes than default 127ms
                melodia_trick=True,         # post-processing for better monophonic melody
            )
        except Exception as e:
            note_events = None  # fall through to pyin

    # ── Fallback: write temp file for Basic Pitch if only array ──
    if note_events is None and audio is not None:
        try:
    ...
    # ── Last resort fallback: pyin ─────────────────────────────
    if note_events is None or len(note_events) == 0:
        return _extract_melody_pyin(audio, sr)

    # ── Convert Basic Pitch note events to our format ──────────
    # note_events is list of (start_time, end_time, midi_note, amplitude)
    # Sort by start time
    note_events = sorted(note_events, key=lambda x: x[0])

    # Build melody sequence (MIDI note numbers) and timestamped pitches
    melody_sequence = []
    timestamped_pitches = []

    for event in note_events:
        start_time, end_time, midi_note, amplitude = event[0], event[1], event[2], event[3]
        midi_note = int(midi_note)
        melody_sequence.append(midi_note)
        # Convert MIDI to Hz for timestamped pitches (used by chord-relation analysis)
        hz = 440.0 * (2.0 ** ((midi_note - 69) / 12.0))
        timestamped_pitches.append((float(start_time), float(hz)))

    # ── Build note name list ───────────────────────────────────
    melody_notes = []
    for note in melody_sequence:
        try:
            note_name = librosa.midi_to_note(note)
            # Convert unicode sharp to ASCII for normalise_enharmonic compatibility
