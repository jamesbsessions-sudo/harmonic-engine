...
def build_midi_from_analysis(wav_path: str, song_id: str, analysis: dict) -> str:
    ...
    try:
        ...
        if "drums" in stems:
            ...
            # Build the 16th note grid for snapping
            sixteenth_sec = beat_duration / 4.0
            track_duration = len(y_drums) / sr_d
            grid_16ths = np.arange(0, track_duration, sixteenth_sec)

            def snap_to_grid(onset_times, grid, tolerance_fraction=0.4):
                """Snap onset times to nearest 16th grid position.
                Remove duplicates (two onsets snapping to the same grid point)."""
                snapped = set()
                for t in onset_times:
                    diffs = np.abs(grid - t)
                    nearest_idx = int(np.argmin(diffs))
                    # Only keep if within tolerance of the grid point
                    if diffs[nearest_idx] < sixteenth_sec * tolerance_fraction:
                        snapped.add(nearest_idx)
                return sorted(snapped)

            def grid_idx_to_tick(idx):
                return idx * ticks_per_16th

            # ── Kick: detect, snap to grid, filter ghost hits ──
            sos_k = butter(4, [20 / nyq, 150 / nyq], btype='band', output='sos')
            y_kick = sosfilt(sos_k, y_drums)
            kick_times = librosa.onset.onset_detect(y=y_kick, sr=sr_d, units='time')

            # Filter kick by onset strength — keep hits above 40th percentile
            kick_env = librosa.onset.onset_strength(y=y_kick, sr=sr_d)
            kick_frames = librosa.onset.onset_detect(y=y_kick, sr=sr_d)
            if len(kick_frames) > 0:
                k_strengths = [kick_env[f] if f < len(kick_env) else 0 for f in kick_frames]
                if k_strengths:
                    k_thresh = np.percentile(k_strengths, 40)
                    strong_kick_frames = [f for f, s in zip(kick_frames, k_strengths) if s >= k_thresh]
                    kick_times = librosa.frames_to_time(np.array(strong_kick_frames), sr=sr_d)

            kick_grid = snap_to_grid(kick_times, grid_16ths)
            for idx in kick_grid:
                drum_events.append((grid_idx_to_tick(idx), KICK_NOTE, 100))

            # ── Snare: detect strong hits, snap to grid ────────
            sos_s = butter(4, [200 / nyq, 1000 / nyq], btype='band', output='sos')
            y_snare = sosfilt(sos_s, y_drums)
            snare_env = librosa.onset.onset_strength(y=y_snare, sr=sr_d)
            snare_frames = librosa.onset.onset_detect(y=y_snare, sr=sr_d)

            if len(snare_frames) > 0:
                s_strengths = [snare_env[f] if f < len(snare_env) else 0 for f in snare_frames]
                if s_strengths:
                    s_thresh = np.percentile(s_strengths, 60)
                    strong_snare_frames = [f for f, s in zip(snare_frames, s_strengths) if s >= s_thresh]
                    snare_times = librosa.frames_to_time(np.array(strong_snare_frames), sr=sr_d)
                else:
                    snare_times = np.array([])
            else:
                snare_times = np.array([])

            snare_grid = snap_to_grid(snare_times, grid_16ths)
            for idx in snare_grid:
                drum_events.append((grid_idx_to_tick(idx), SNARE_NOTE, 100))

            # ── Hi-hat: detect, snap to grid ───────────────────
            sos_h = butter(4, 6000 / nyq, btype='high', output='sos')
            y_hat = sosfilt(sos_h, y_drums)
            hat_times = librosa.onset.onset_detect(y=y_hat, sr=sr_d, units='time')

            hat_grid = snap_to_grid(hat_times, grid_16ths)
            for idx in hat_grid:
                # Lower velocity for hats that fall between beats (ghost hats)
                beat_position = (idx % 4)  # 0=downbeat, 2=upbeat
                vel = 80 if beat_position == 0 else 65 if beat_position == 2 else 50
    ...
    # Reconstruct beat times from tempo
...
async def export_midi(file: UploadFile = File(...)):
    ...
    try:
        ...
        # Clean up after a delay to let the response send
        import threading
        def cleanup():
            import time
            time.sleep(5)
