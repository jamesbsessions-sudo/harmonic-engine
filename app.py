...
def extract_melody(audio: np.ndarray, sr: int = 44100, audio_path: str = None):
    ...
    Basic Pitch outputs actual note events (onset, offset, pitch) rather than
    frame-level f0 estimates, so it handles note segmentation with a trained
    neural network instead of relying on heuristic post-processing.
    
    Falls back to pyin if Basic Pitch isn't available or audio_path isn't provided.
    ...
    # ── Run primary Basic Pitch extraction ─────────────────────
    note_events = None
    if resolve_path:
    ...
    # ── Run variant parameter sets for comparison ──────────────
    variants = []
    if resolve_path:
        variant_configs = [
            {
        ...
        for cfg in variant_configs:
            try:
                ...
                if v_events:
                    v_seq, _ = _note_events_to_sequence(v_events)
                    v_notes = _sequence_to_notes(v_seq)
                    variants.append({
                        "label": cfg["label"],
                        "params": {
                            "onset_threshold": cfg["onset_threshold"],
    ...
    return {
        ...
        # Variant comparison for parameter tuning
        "melody_variants": variants,
